...
- Scope: build only what the current milestone needs (see `docs/design.md`). Do not add
  ...
  whole-object PUT (and a HEAD/GET that asks for it with `x-amz-checksum-mode: ENABLED`), and user metadata
  ...
  a pass count is not a measure of a store. SHA-256, CRC32, CRC64NVME, a trailing checksum, and a
  ...
  checked, because the write already hashes the body to make the ETag.
