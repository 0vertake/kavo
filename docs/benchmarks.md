...
| manifest commit to etcd (`meta.Commit`) | 0.47 ms |
...
| --- | --- | --- |
| PUT 4 KB | 25 ms | 19 ms |
| PUT 1 MB | 30 ms / 35 MB/s | 24 ms / 44 MB/s |
| PUT 64 MB | 197 ms / 341 MB/s | 151 ms / 444 MB/s |
| GET 4 KB | 0.65 ms | 0.15 ms |
| GET 1 MB | 1.7 ms / 607 MB/s | 0.57 ms / 1.8 GB/s |
| GET 64 MB | 46 ms / 1.5 GB/s | 22 ms / 3.0 GB/s |

Every PUT byte is written three times, so 444 MB/s of client throughput is 1.3 GB/s of durable
writes on one disk. Reads scale to 3.0 GB/s and the local store alone reads at 3.8 GB/s, so reads
are bound by the disk and the loopback, not by the code.

Writes barely improve with concurrency (25 ms → 19 ms across 8 clients) because the barriers
...
| operation | one client | 8 clients |
| --- | --- | --- |
| PUT 4 KB | 27 ms | 20 ms |
| PUT 1 MB | 31 ms / 33 MB/s | 24 ms / 44 MB/s |
| PUT 64 MB | 186 ms / 361 MB/s | 127 ms / 530 MB/s |
| GET 4 KB | 0.62 ms | 0.14 ms |
| GET 1 MB | 1.5 ms / 686 MB/s | 0.49 ms / 2.2 GB/s |
| GET 64 MB | 46 ms / 1.5 GB/s | 21 ms / 3.3 GB/s |
| GET an 8 MB range of a 64 MB object | 22 ms / 388 MB/s | |
| HEAD | 0.46 ms | |
| PUT 64 MB as 8 multipart parts | 380 ms / 177 MB/s | |
| `ListObjectsV2`, a page of 1000 keys | 14 ms | |
| `ListObjectsV2`, the same keyspace under a delimiter | 5.3 ms | |
...
| operation | rate |
| --- | --- |
| heal a lost disk, unrated (`RepairHeal`) | 319 MB/s |
| scrub, unrated (`Scrub`) | 2.1 GB/s |
| survey a healthy partition (`RepairSurvey`) | 202 µs per copy |
...
| PUT 4 KB | 25 ms | 42 ms |
| PUT 1 MB | 30 ms / 35 MB/s | 46 ms / 23 MB/s |
| PUT 64 MB | 145 ms / 464 MB/s | 190 ms / 354 MB/s |
| GET 64 MB | 47 ms / 1.4 GB/s | 66 ms / 1.0 GB/s |
| GET 64 MB, two shards gone | — | 64 ms / 1.0 GB/s |
| allocated per 64 MB GET | 125 KB | 168 MB |

A large coded write costs 31% more wall time and stores half the bytes, which is the trade the mode
exists to make. Reads are ~29% slower, and **a degraded read is no slower than a healthy one** —
...
Small writes are where coding hurts: 42 ms against 25 ms, because a 4 KB object still becomes six
...
| --- | --- | --- |
| one-chunk manifests (small objects) | 6.5 ms | 2.0 MB |
| 32-chunk manifests (1 GB objects) | 41 ms | 6.3 MB |
...
- **Batching the repair survey** into one "which of these do you have?" per node. At 202 µs per
  copy, a pass over 10 M chunks is ~1.7 hours of pure survey. That is a real problem at that
...
measurement: chunking runs at 5.7 GB/s against replication's ~360 MB/s, so the read is 6% of a large
